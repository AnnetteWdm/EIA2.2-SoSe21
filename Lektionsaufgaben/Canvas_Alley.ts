namespace L08_Canvas_Alley {
    ...
    function handleLoad(_event: Event): void {
        ...
        //Aufrufe der Funktionen in der Reihenfolge von hinten nach vorne//
        drawBackground();
        drawSun({ x: 100, y: 75 });
        ...
        drawTrees(8, posTreesStart, posTreesEnd, 0.1, 0.37, 1.4); //erste Baumreihe
        ...
        drawTrees(8, posTreesStart, posTreesEnd, 0.1, 0.37, 1.4); //Zweite Baumreihe
    }

    function drawTrees(_nTrees: number, _posStart: Vector, _posEnd: Vector, _minScale: number, _stepPos: number, _stepScale: number): void { //stepPos ist nächster Schritt zum nächsten Baum, posCurrent
        ...
        crc2.translate(_posStart.x, _posStart.y); //Koordinatensystem an die ursprüngliche Position
        crc2.scale(_minScale, _minScale); //Skalierung, da die Bäume nach hinten kleiner werden. Angefangen wird mit dem kleinsten Koordinatensystem/Baum
        ...
        } while (--_nTrees > 0); // Anzahl der Bäume wird runtergezählt: Es werden solange Bäume gezeichnet, bis alle gezeichnet wurden.
    ...
    function drawTree(): void {
        ...
        crc2.save(); //save transform
        crc2.translate(0, -120); 

        do {
            let y: number = Math.random() * 350; // Y von 0-350
            let size: number = 1 - y / 700; //Größe kann max 0,5 werden. -> Wert zw. 0,5 und 1 -> Nach oben hin werden die Büsche kleiner, da y mit in die Größe spielt
